...
*  **Params:**
	...
	**Optional:**
	...
	`term_date [date]` - terminate date.

	`position_text [string]` - position text.

	`nok_name_text [string]` - next of kin name.

	`nok_relat_text [string]` - next of kin namerelationship.

	`nok_add1_text [string]` - next of kin address 1.

	`nok_add2_text [string]` - next of kin address 2.
...
* **Success Response:**
	...
	"timestamp": "{ts '2019-06-28 12:09:27'}"
	}
	```
...
* **Error Response:**
	...
	`state_text` is not passed in if country is Australia
	```javascript
	"__invalid": {
		"employee": {
			"[id]": {
	...
	`post_code` is not passed in if country is Australia
	```javascript
	"__invalid": {
		"employee": {
			"[id]": {
	...
	`state_text` is passed in if country is not Australia
	```javascript
	"__invalid": {
		"employee": {
			"[id]": {
	...
	`post_code` is passed in if country is not Australia
	```javascript
	"__invalid": {
		"employee": {
			"[id]": {
